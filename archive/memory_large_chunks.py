...
class Memory:
    ...
    def import_text(self, file_path: str, block_size: int = 500, overlap: int = 50):
        ...
        human_timestamp = self._get_human_timestamp()
        enriched = f"[{human_timestamp}] {corpus}"
        blocks = self._segment_text(enriched, block_size, overlap)

        total = len(blocks)
        added = 0
        skipped = 0

        print(f"Processing {total} blocks from {file_path}...")
        for i, block in enumerate(blocks, 1):
            ...
            if i % 100 == 0 or i == total:
                percent = (i / total) * 100
                print(f"Imported {i}/{total} blocks ({percent:.1f}%) | Added: {added}, Skipped (duplicates): {skipped}")

        print(f"\n✅ Import complete: {added} new blocks added, {skipped} duplicates skipped.\n")
