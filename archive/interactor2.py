    ...
    def interact(self, user_input: Optional[str], stream: bool = True, markdown: bool = False) -> Optional[str]:
        ...
        while True:
            params = {
                ...
                "stream": use_stream,
                **({"tools": self.tools, "tool_choice": "auto"} if self.is_ollama and self.tools_enabled else {"functions": self.tools} if self.tools_enabled else {})
            }

            try:
                #console.print(f"[yellow]Sending request: {user_input}[/yellow]")
                ...
                if use_stream:
                    ...
                    for chunk in response:
                        ...
                        if self.tools_enabled:
                            ...
                            if call_data:
                                #console.print(f"[yellow]Tool call detected: {call_data}[/yellow]")
                                if call_data.name and not current_call["name"]:
                                    current_call["name"] = call_data.name
                                if call_data.arguments:
                                    current_call["arguments"] += call_data.arguments
                                if self.is_ollama and hasattr(call_data, "id") and not current_call["id"]:
                                    current_call["id"] = call_data.id
