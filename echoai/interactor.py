...
class Interactor:
    ...
    def interact(
        ...
        try:
            ...
            if use_stream:
                tool_call_data = {"name": "", "arguments": "", "id": None}
                    ...
                    if self.tools_enabled:
                        if self.is_ollama and delta.tool_calls:
                            ...
                            if tool_call.function.name:
                                tool_call_data["name"] = tool_call.function.name
                            ...
                            if delta.function_call.name:
                                tool_call_data["name"] = delta.function_call.name
                            if delta.function_call.arguments:
                                tool_call_data["arguments"] += delta.function_call.arguments
                            is_collecting_tool_args = True

                    if finish_reason in ["tool_calls", "function_call"] and is_collecting_tool_args:
                        full_content = self._handle_tool_call(
                            tool_call_data["name"],
                ...
                if tool_calls:
                    ...
                    full_content = self._handle_tool_call(function_name, arguments, tool_call_id, params, markdown)
                else:
