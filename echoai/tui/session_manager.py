...
class SessionManager:
    def __init__(self, theme_name="default"):
        ...
        self.body = urwid.ListBox(self.body_widget)
    ...
    def load_sorted_sessions(self):
        ...
        return sorted(sessions, key=lambda s: s.get("updated", s["created"]), reverse=True)

    def refresh_sessions(self):
        self.sessions = self.load_sorted_sessions()
        self.filtered_sessions = (
            self.session_mgr.search_meta(self.active_query)
            if self.active_query else self.sessions
        )
    ...
    def build_session_list(self):
        ...
        for i, s in enumerate(self.filtered_sessions):
            ...
            text = urwid.Text("> " + label if i == self.current_index else "  " + label)
            style = 'highlight' if i == self.current_index else 'default'
            items.append(urwid.AttrMap(text, style))
        return items
    ...
    def build_preview(self, session_id):
        ...
        widgets = [
            urwid.AttrMap(urwid.Text(line[1]), line[0]) if isinstance(line, tuple)
            else urwid.Text(line)
            for line in lines
        ]
    ...
    def build_editable_info_view(self, session_id):
        ...
        self.edit_widgets = {
            'name': urwid.Edit(('prompt', 'Name: '), name),
            'tags': urwid.Edit(('prompt', 'Tags (comma separated): '), tags),
            'summary': urwid.Edit(('prompt', 'Summary: '), summary)
        }
        meta_lines = [
            urwid.Text(f"ID: {session.get('id', '')}"),
            urwid.Divider(),
            urwid.AttrMap(self.edit_widgets['name'], 'input'),
            urwid.AttrMap(self.edit_widgets['tags'], 'input'),
            urwid.AttrMap(self.edit_widgets['summary'], 'input'),
            urwid.Divider(),
        ...
        msgs = self.session_mgr.load(session_id)
        role_counts = Counter(msg.get('role', 'unknown') for msg in msgs)
        meta_lines.append(urwid.AttrMap(urwid.Text("Message Stats"), 'highlight'))
        meta_lines.append(urwid.Text(f"Branches: {len(session.get('branches', []))}"))
        meta_lines.append(urwid.Text(f"Total Messages: {len(msgs)}"))
        for role, count in sorted(role_counts.items()):
            meta_lines.append(urwid.Text(f"{role.title()} Messages: {count}"))

        self.info_focus = urwid.SimpleFocusListWalker(meta_lines)
        return urwid.ListBox(self.info_focus)


    def update_footer(self):
        msg = {
            "main": "[↑↓] [→] info [enter] load [v]iew [d]elete [r]ename [n]ew [/] search [esc] exit",
            "preview": "[↑↓] [space] pgdown  [esc] back",
            "rename": "[enter] confirm  [esc] cancel",
            "new": "[enter] create  [esc] cancel",
            "delete_confirm": "[y] confirm  [n/esc] cancel",
            "search": "[enter] run search  [esc] cancel",
            "info": "[↑↓|tab] nav  [enter] save  [esc] back"
        }.get(self.mode, "[esc] back")
        self.footer_text.set_text(('footer', msg))
    ...
    def build_input_overlay(self, prompt, callback):
        edit_widget = urwid.Edit(("input", f"{prompt}: "), edit_text=self.input_text)
        wrapped = urwid.LineBox(urwid.Padding(urwid.AttrMap(edit_widget, 'input'), left=1, right=1))
        ...
        def input_handler(key):
            if key == 'enter':
                self.input_text = edit_widget.edit_text.strip()
                callback(self.input_text)
                self.return_to_main()
            elif key == 'esc':
                self.return_to_main()
        return overlay, input_handler

    def handle_input(self, key):
        if key == 'window resize':
            self.header_obj.resize(self.screen.get_cols_rows()[0])
            return
        ...
        if self.mode == 'main':
            if key in ('up', 'k'):
                ...
                self.mode = 'info'
                sid = self.filtered_sessions[self.current_index]['id']
                self.main_frame.body = self.build_editable_info_view(sid)
                self.update_footer()
            elif key == '/':
                self.mode = 'search'
                ...
                self.mode = 'rename'
                sid = self.filtered_sessions[self.current_index]['id']
                current_name = next((s['name'] for s in self.sessions if s['id'] == sid), '')
                self.input_text = current_name
            ...
            if key == 'esc':
                self.return_to_main()
            elif key in (' ', 'page down') and self.preview_box:
                pos = self.preview_box.get_focus()[1]
            ...
            if key == 'esc':
                ...
                self.footer_text.set_text(('footer', f"✓ Changes to session '{name}' saved. Press ESC to return."))
            elif key in ('tab', 'down'):
                self.info_focus.set_focus(min(len(self.info_focus) - 1, self.info_focus.get_focus()[1] + 1))
            elif key == 'up':
                self.info_focus.set_focus(max(0, self.info_focus.get_focus()[1] - 1))
            return
    ...
    def search_sessions(self, query):
        ...
        self.current_index = 0
        self.body_widget[:] = self.build_session_list()
        self.main_frame.body = urwid.ListBox(self.body_widget)
